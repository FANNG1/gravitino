...
import org.apache.gravitino.maintenance.optimizer.common.reader.FileContentReader.Source;
...
public class TableRegister {
  ...
  public void registerFromFile(String filePath) {
    try (FileContentReader<? extends Source> reader = getFileContentReader(filePath)) {
      // list all files
      for (Source source : reader.listSources(filePath)) {
        try (BufferedReader bufferedReader = getReaderForSource(reader, source)) {
          LOG.info("Processing: {}", source.getPath());
          // read file content
  ...
  private FileContentReader<? extends Source> getFileContentReader(String filePath) {
    return S3Utils.isS3Path(filePath) ? new S3FileReader(s3Region) : new LocalFileReader();
  }

  @SuppressWarnings("unchecked")
  private static <S extends Source> BufferedReader getReaderForSource(
      FileContentReader<S> reader, Source source) {
    return reader.getReader((S) source);
  }
  ...
  private void createTable(TableRegistrationRequest request) {
    ...
    Map<String, String> createProperties = buildCreateProperties(icebergTable.properties());
  ...
  @VisibleForTesting
  static Map<String, String> buildCreateProperties(Map<String, String> sourceProperties) {
    Map<String, String> createProperties = new HashMap<>();
    if (sourceProperties != null) {
      createProperties.putAll(sourceProperties);
    }
    createProperties.put(Table.PROPERTY_TABLE_FORMAT, "iceberg");
